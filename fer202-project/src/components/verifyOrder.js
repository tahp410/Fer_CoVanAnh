...
function VerifyOrder() {
  ...
  const handleSubmit = (e) => {
    ...
    const confirmInfo = window.confirm(
      `Bạn có chắc chắn rằng các thông tin của mình là chính xác, đặc biệt là Email ??`
    );
    if (!confirmInfo) {
    ...
    <Container fluid>
      ...
      {cart.length === 0 ? (
        ...
        >
          ...
          <Link to={"/productuser"}>Quay lại Shop ngay 🛒💻📱</Link>
        </Alert>
      ) : (
        <>
          <Row className="mt-3 mb-3">
            ...
            >
              (Vì bạn không có tài khoản nên chúng tôi cần thực hiện các bước
              xác minh để chắc chắn rằng bạn có thể nhận được sản phẩm khi mua
                ...
                <Col>
                  <div>
                    ...
                    <span style={{ color: "red", paddingBottom: "20px" }}>
                      Nhấn "Thanh Toán" đồng nghĩa với việc bạn đã đồng ý với
                      các thông tin của mình là chính xác. Có thể bạn sẽ muốn
                      kiểm tra lại Email.
                      <Link
                        to={"/auth/login"}
                      ...
                      ngay để có thể xem Lịch Sử Đặt Hàng và sử dụng các Voucher
                      từ Shop
                    </span>
                  </div>
                </Col>
